package me.TheJokerDev.other.nms;
...
import me.TheJokerDev.other.utils.Common;
import me.TheJokerDev.other.utils.reflect.*;
...
public class PacketHandler extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext channelHandlerContext, Object packet) throws Exception {
        if (packet != null && packet.getClass().isAssignableFrom(ENTITY_USE_PACKET_CLASS)) {
            ...
            HoloActions clickType = this.getClickType(packet);
            Main.getManager().onClick(player, entityId, clickType);
        }
    ...
    private HoloActions getClickType(Object packet) {
        int ordinal = this.getEntityUseActionOrdinal(packet);
        if (ordinal == 1) {
            return player.isSneaking() ? HoloActions.SHIFTING : HoloActions.LEFT_CLICK;
        } else return player.isSneaking() ? HoloActions.SHIFTING : HoloActions.RIGHT_CLICK;
    }
}